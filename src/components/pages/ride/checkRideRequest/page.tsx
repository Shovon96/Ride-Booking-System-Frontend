...
export default function CheckRideRequestPage() {
  const { data: driverData, isLoading } = useUserDataQuery(undefined);
  ...
  const fetchRideRequests = useCallback(async () => {
    ...
    try {
      ...
      setRideRequests(formattedRides.filter((r: any) => r.status === "REQUESTED"));
  ...
  const toggleAvailability = async () => {
    if (["NOTAPPROVED", "RIDING", "SUSPENDED"].includes(
      driverStatus as "NOTAPPROVED" | "RIDING" | "SUSPENDED"
    ))
      return;
    try {
      const res = await toggleDriverStatus({ driverStatus: driverStatus === "AVAILABLE" ? "UNAVAILABLE" : "AVAILABLE" }).unwrap();
